<?php $award_data = $this->db->get_where('award', array('award_code' => $param2))->result_array();
                            ...
                            <select name="user_id" class="form-control select2" required>
                                ...
                                <optgroup label="<?php echo get_phrase('teacher'); ?>">
                                    ...
                                    foreach ($teacher as $key => $teacher) : ?>
                                        <option <?php if ($award['user_id'] == 'teacher-' . $teacher['teacher_id']) echo "selected"; ?> value="teacher-<?php echo $teacher['teacher_id']; ?>"><?php echo $teacher['name']; ?></option>
                                    <?php endforeach; ?>
                                ...
                                <optgroup label="<?php echo get_phrase('hrm'); ?>">
                                    ...
                                    foreach ($hrm as $key => $hrm) : ?>
                                        <option <?php if ($award['user_id'] == 'hrm-' . $hrm['hrm_id']) echo "selected"; ?> value="hrm-<?php echo $hrm['hrm_id']; ?>"><?php echo $hrm['name']; ?></option>
                                    <?php endforeach; ?>
                                ...
                                <optgroup label="<?php echo get_phrase('accountant'); ?>">
                                    ...
                                    foreach ($accountant as $key => $accountant) : ?>
                                        <option <?php if ($award['user_id'] == 'accountant-' . $accountant['accountant_id']) echo "selected"; ?> value="accountant-<?php echo $accountant['accountant_id']; ?>"><?php echo $accountant['name']; ?></option>
                                    <?php endforeach; ?>

                                <optgroup label="<?php echo get_phrase('parent'); ?>">
                                    <?php $parent = $this->db->get('parent')->result_array();
                                    foreach ($parent as $key => $parent) : ?>
                                        <option <?php if ($award['user_id'] == 'parent-' . $parent['parent_id']) echo "selected"; ?> value="parent-<?php echo $parent['parent_id']; ?>"><?php echo $parent['name']; ?></option>
                                    <?php endforeach; ?>

                                <optgroup label="<?php echo get_phrase('hostel'); ?>">
                                    <?php $hostel = $this->db->get('hostel')->result_array();
                                    foreach ($hostel as $key => $hostel) : ?>
                                        <option <?php if ($award['user_id'] == 'hostel-' . $teacher['hostel_id']) echo "selected"; ?> value="hostel-<?php echo $hostel['hostel_id']; ?>"><?php echo $hostel['name']; ?></option>
                                    <?php endforeach; ?>
                                ...
                                <optgroup label="<?php echo get_phrase('librarian'); ?>">
                                    ...
                                    foreach ($librarian as $key => $librarian) : ?>
                                        <option <?php if ($award['user_id'] == 'librarian-' . $librarian['librarian_id']) echo "selected"; ?> value="librarian-<?php echo $librarian['librarian_id']; ?>"><?php echo $librarian['name']; ?></option>
                                    <?php endforeach; ?>
